...
<%@page import="app.societywala.account.bo.CreditNotesEntryDetailsBO"%>
<%@page import="app.societywala.account.bo.SocietyCreditNotesEntryBO"%>
...
SocietyCreditNotesEntryBO societyJournalEntryBO =(SocietyCreditNotesEntryBO)request.getAttribute("societyJournalEntryBO");
List<CreditNotesEntryDetailsBO> journalEntryByDetailsList = (List<CreditNotesEntryDetailsBO>)request.getAttribute("journalEntryByDetailsList");
List<CreditNotesEntryDetailsBO> journalEntryToDetailsList = (List<CreditNotesEntryDetailsBO>)request.getAttribute("journalEntryToDetailsList");
	...
	<nav class="navbar-static-top">
  	<div id="subNavBar" class="navbar-collapse collapse in">
          <ul class="nav navbar-nav">
			...
			<li>
				<a  href="javascript:;"  onclick="javascript:addNewJournalEntry();"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;<spring:message code="account.manage.credit.notes.entry.page.btn.add.journalEntry"/></a>
			</li>
			<%} %>
			<li>
	 	...
	 	<div class="row">
    		<div class="col-md-12">
    				<form id="journalReceiptForm" method="post">
						 <input type="hidden" id="voucherId" name="voucherId" value="<%=societyJournalEntryBO.getCreditNotesEntryId()%>">
					</form>
    				<div class="box box-primary">
    					<form:form  cssClass="form"  style="width:100%;" method="post" commandName="societyJournalEntryBO">
						<form:hidden path="creditNotesEntryId"/>
						...
						<div class="box-body">
												...
												<c:forEach items="${journalEntryByDetailsList}" var="journalEntryByDetails" varStatus="counter">
											 		<tr id="journalEntryByDetailsBOTR${counter.count}">
											 			...
											 			<%}else  if (pageRenderMode.equals(CommonConstant.EDIT_MODE)) {%>
											 				...
											 				<input type="hidden" name="journalEntryByDetails${counter.count}" id="journalEntryByDetails${counter.count}" value="${journalEntryByDetails.creditNotesEntryDetailsId}">
									...
									<td>		
										...
										<table class="table table-bordered dt-responsive" id="journalEntryToTbl">
											<thead>
												...
												<tr>
													<td width="60%" >
														<label class="control-label required">All (Gain/Giver/Given)</label>
													</td>	
													<td width="40%">
														<label class="control-label required"><spring:message code="account.journal.entry.form.page.lable.rupees"/></label>
													</td>					
												...
												<c:forEach items="${journalEntryToDetailsList}" var="journalEntryToDetails" varStatus="counter">
											 		<tr id="journalEntryToDetailsBOTR${counter.count}">
											 			...
											 			<%}else  if (pageRenderMode.equals(CommonConstant.EDIT_MODE)) {%>
											 				...
											 				<input type="hidden" name="journalEntryToDetails${counter.count}" id="journalEntryToDetails${counter.count}" value="${journalEntryToDetails.creditNotesEntryDetailsId}">
							...
							<div class="form-group">
									<form:label cssClass="control-label required" path="creditNotesEntryDate">
										<spring:message code="account.credit.notes.entry.form.page.lable.JournalEntry.Date"/>
									</form:label>
									<div class="input-group date">
					                  <div class="input-group-addon">
					                    <i class="fa fa-calendar"></i>
					                  </div>
					                  <form:input cssClass="form-control pull-right datepicker" path="creditNotesEntryDate" dateFormat="dateFormat" required="required"/>
				                  	</div>												
							</div>							
							<div class="form-group">
...
<script type="text/javascript">
	bindRestrictedDatePicker('creditNotesEntryDate',minJqueryDateFmtStr,maxJqueryDateFmtStr);
	var journalEntityType = '<%=JournalEntityType.CREDIT_NOTES_ENTRY%>';
</script>
<script type="text/javascript" src="<%=jsRoot%>/account/creditNotesEntryVoucherPage.js?${Server_Token_Value}"></script>
<script type="text/javascript" src="<%=jsRoot%>/account/creditNotesEntryReceiptPage.js?${Server_Token_Value}" ></script>
</body>
